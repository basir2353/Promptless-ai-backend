...
export class ContextRetrieverService {
   ...
   */
  async buildLLMContext(userId: string, userQuery: string, limit: number = 3): Promise<string> {
    const queryVector = await this.embeddingService.generateEmbedding(userQuery);
    const results = await this.qdrantService.searchSimilar(userId, queryVector, limit);
    ...
    const memoryStrings = results
      .map((item: any, idx: number) => `- [Memory ${idx + 1}]: ${item.payload?.text || ''}`)
      .join('\n');
